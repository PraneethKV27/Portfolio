...
export default function Navbar({ activeSection, setActiveSection, toggleAudio, audioPlaying, toggleDarkMode, darkMode }) {
          ...
          {navItems.map((item) => {
            ...
            return (
              <a 
                ...
                className={`font-tech font-medium transition-all duration-300 relative group text-base cursor-pointer ${
                  isActive ? 'text-cyan-400 text-glow-cyan' : 'text-gray-300 hover:text-cyan-400'
                }`}
              >
                {item.label}
                <span className={`absolute bottom-[-4px] left-0 h-[2px] bg-cyan-400 transition-all duration-300 ${isActive ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
              </a>
            );
          })}
        </nav>
            ...
            {navItems.map((item) => {
              ...
              return (
                <a 
                  ...
                  href={item.href} 
                  onClick={(e) => handleNavClick(e, item.label)}
                  className={`font-tech font-semibold tracking-wider p-2 border-b border-cyan-500/5 cursor-pointer ${
